...
export class Auth2Session {
    ...
    getAuthCookie(): string | null {
        const cookies = this.cookieManager.getItems(this.cookieName);
        ...
        if (this.extraCookies) {
            this.extraCookies.forEach((cookieConfig) => {
                ...
                if (cookieConfig.domain) {
                    extraCookie.setDomain(cookieConfig.domain);
                }
    ...
    removeSessionCookie(): void {
        ...
        if (this.extraCookies) {
            this.extraCookies.forEach((cookieConfig) => {
                const cookieToRemove = new Cookie(
                    cookieConfig.name,
                    ''
                ).setPath('/');
                if (cookieConfig.domain) {
                    cookieToRemove.setDomain(cookieConfig.domain);
                }

                this.cookieManager.removeItem(cookieToRemove);
            });
        }
    }
