...
export default class SelectNarrative extends Component<SelectNarrativeProps, SelectNarrativeState> {
    ...
    renderNarrativeSelect() {
        const options = this.props.narratives.map(({ ref, title, version }) => {
            ...
            return <option value={ref} key={ref}>{title} (v {version})</option>;
        });
        options.unshift(<option value='' key='none'>- Please select a static narrative -</option>)
        return <Form.Select
    ...
    renderSelectedNarrative() {
        if (!this.props.selectedNarrative) {
            return <AlertMessage variant="info">Please select a public Narrative above</AlertMessage>;
        }
