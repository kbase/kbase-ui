...
export default class AuthorsSection extends Component<AuthorsSectionProps, AuthorsSectionState> {
    ...
    renderAuthors2() {
        if (this.props.authors.length === 0) {
            return <AlertMessage variant="warning">
                You must provide one or more authors.
            </AlertMessage>
        }
        ...
        const columns: Array<ColumnDef<EditableAuthor>> = [
            {
                ...
                render: (author: EditableAuthor) => {
                    if (author.firstName.isValid() && author.middleName.isValid() && author.lastName.isValid() &&
                        author.institutionField.isValid() && author.emailAddressField.isValid() &&
                ...
                render: (author: EditableAuthor) => {
                    switch (author.firstName.fieldState.status) {
                        ...
                        case FieldStatus.UNTRANSFORMABLE:
                            return <div className="bg-danger text-center" ><span className="fa fa-exclamation-circle fa-inverse" /></div>
                        case FieldStatus.VALID:
                            return <span>{author.firstName.fieldState.editValue}</span>
                    }
                },
                sorter: (a: EditableAuthor, b: EditableAuthor) => {
                    return optionalStringComparator(a.firstName.getEditValue(), b.firstName.getEditValue());
                }
            },
            {
                id: 'lastName',
                ...
                sorter: (a: EditableAuthor, b: EditableAuthor) => {
                    return optionalStringComparator(a.lastName.getEditValue(), b.lastName.getEditValue());
                }
            },
            // {
            //     id: 'orcidId',
            ...
            // },
            {
                ...
                sorter: (a: EditableAuthor, b: EditableAuthor) => {
                    return a.contributorType.getEditValue().localeCompare(b.contributorType.getEditValue());
                }
            },
            {
                id: 'buttons',
                ...
                render: (author: EditableAuthor) => {
                    return <span>
                        <Button variant="outline-primary" style={{ border: 'none', padding: '0.25em' }} onClick={(e) => { e.stopPropagation(); this.props.onEdit(author) }}>
                            <span className="fa fa-edit" />
                        </Button>
                        <Button variant="outline-danger" style={{ border: 'none', padding: '0.25em' }} onClick={(e) => { e.stopPropagation(); this.props.onDelete(author) }}>
        ...
        return <DataBrowser<EditableAuthor>
            heights={{
                header: 40,
                row: 40
            }}
    ...
    renderAuthorEditor() {
        if (!(this.props.selected)) {
            return <AlertMessage variant="info">
    ...
    isCompletable() {
        return this.props.authors.every((author) => {
            return (author.firstName.isValid() && author.middleName.isValid() && author.lastName.isValid() &&
                author.institutionField.isValid() && author.emailAddressField.isValid() &&
                author.orcidIdField.isValid() &&
                author.contributorType.isValid());
            ...
            <Row className="g-0">
                <Col md={12} className="mt-2">
                    <Row style={{ justifyContent: 'center' }} className="g-0">
                        <Button variant="primary" className="w-auto" onClick={this.props.onDone} disabled={!this.isCompletable()}>Done</Button>
                    </Row>
                </Col>
            </Row>
        </Stack>
