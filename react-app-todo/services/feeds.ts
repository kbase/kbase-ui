...
export interface FeedsNotification {
...
export default class FeedsService {
    ...
    state: DataState<FeedsNotification>;

    constructor({ config, runtime }: FeedsServiceParams) {
        ...
        this.disabled = config.ui.coreServices.disabled.includes('Feeds');
    ...
    start() {
        return tryPromise<void>(() => {
            ...
            if (this.disabled) {
                console.warn(
                    'Feeds service disabled; skipping monitoring hooks'
                );
                return;
            }

            // listen for login and out events...
            this.runtime.on('session', 'loggedin', () => {
                this.startFeedsMonitoring();
            });

            this.runtime.on('session', 'loggedout', () => {
    ...
    monitoringLoop() {
        ...
        const monitoringJob = () => {
            const feedsClient = new Feeds({
                url: this.runtime.config('services.Feeds.url', null),
            ...
            return feedsClient
                ...
                .then(({ unseen: { global, user } }) => {
                    ...
                    const unseenNotificationsCount = this.runtime
                        .db()
                        .get('feeds.unseenNotificationsCount', 0);
