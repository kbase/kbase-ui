...
export default class NarrativeLoader extends Component<NarrativeLoaderProps, NarrativeLoaderState> {
    ...
    if (!narrativeIdRaw) {
      this.setState({
        loadState: {
          ...
          message: "The Narrative Loader requires the 'n' search parameter, which was not provided in the URL.",
        },
      });
      return;
    }
  ...
  async tryLoading(narrativeId: number) {
    ...
    try {
      ...
      for (let tries = 1; tries <= MAX_TRIES; tries += 1) {
        const retry = await this.checkNarrative(this.state.loadState.url);
        if (retry) {
          this.setState({
            loadState: {
              status: LoadStatus.POKING,
  ...
  renderOK(loadingState: LoadStateOk) {
    ...
    return (
      <AlertMessage variant="success" style={{ width: '50%', margin: '0 auto' }}>
  ...
  render() {
    switch (this.state.loadState.status) {
      ...
      case LoadStatus.POKING:
        return (
          <Alert itemType="info" style={{ width: '50%', margin: '0 auto' }}>
            ...
            <ProgressBar
              now={(100 * this.state.loadState.tries) / MAX_TRIES}
              label="Waiting for Narrative session..."
            />
          </Alert>
        );
      case LoadStatus.OK:
      ...
      case LoadStatus.INIT_ERROR:
        return (
          <ErrorMessage
            title={this.state.loadState.title}
            message={this.state.loadState.message}
