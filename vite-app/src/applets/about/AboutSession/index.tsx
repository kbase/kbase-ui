...
export default class AboutSessionController extends Component<AboutSessionControllerProps> {
  componentDidMount() {
  ...
  renderRoles(roles: Array<Role>) {
    ...
    return (
      <Table className="w-auto subdued-headers">
        <thead>
          <tr>
            <th>Role Id</th>
            <th>Description</th>
  ...
  renderCustomRoles(roles: Array<string>) {
    ...
    return roles.map((roleId) => {
      return <div key={roleId}>{roleId}</div>;
    });
  }
  ...
  renderAuthenticated(authState: AuthenticationStateAuthenticated) {
    const {
      authInfo: {
        token,
        tokenInfo: { user, expires },
        account: { roles, customroles },
      },
    } = authState;

    const loginToken = (() => {
        if (authState.authInfo.account.roles.some(({id}) => {return id === 'DevToken'})) {
            return <tr>
                <th style={{ width: '8rem' }}>Login Token</th>
                <td>{token}</td>
          </tr>
        }
    })();
    ...
    return (
      <Table className="subdued-headers">
        <tbody>
          {loginToken}
          <tr>
            <th>Expires</th>
  ...
  renderUnauthenticated(_authState: AuthenticationStateUnauthenticated) {
    return <Empty noBorder>You are not signed in, and therefore do not have a KBase Session.</Empty>;
  }
  ...
  render() {
    return (
      <Well variant={this.renderVariant()}>
