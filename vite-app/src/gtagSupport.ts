...
import { Account } from "lib/kb_lib/comm/coreServices/Auth";

type GTagStatus = 'WAITING' | 'PENDING' | 'SEND_NOW' | 'SENDING';

interface WindowWithGTag extends Window {
    dataLayer: Array<any>;
}

const gtagWindow = window as unknown as WindowWithGTag;
...
class GTagSupport {
    ...
    PROD_UI_ORIGIN = 'https://narrative.kbase.us';
    PROD_SERVICE_ORIGIN = 'https://kbase.us';
    GOOGLE_GTAG = 'G-KXZCE6YQFZ';
    GOOGLE_AD_TAG = 'AW-753507180';

    // The sending of the page hit is delayed until the page title stops change for the duration
    // set here.
    TITLE_STABILIZATION_PERIOD = 3000;

    // Keep the auth token cached for up to 10 minutes.
    KBASE_AUTH_TOKEN_TTL = 600000;
    KBASE_AUTH_INFO_EXPIRES_AT: number | null = null;
    KBASE_AUTH_INFO: Account | null = null;
    KBASE_AUTH_TOKEN: string | null = null;



    // Stores the current page title, which is really the last one recorded
    // in the async loop below.
    LAST_PAGE_TITLE: string | null = null;
    ...
    async waitUntilStatus(status: GTagStatus, timeout: number) {
     ...
     */
    getServiceOrigin() {
        if (document.location.origin === this.PROD_UI_ORIGIN) {
            return this.PROD_SERVICE_ORIGIN;
        }
     ...
     */
    pushGTag(_tag: string, _value: any, _options?: any) {
     ...
     */
    async fetchAuth(token: string) {
        if (
            this.KBASE_AUTH_INFO !== null &&
            this.KBASE_AUTH_INFO_EXPIRES_AT !== null &&
            this.KBASE_AUTH_INFO_EXPIRES_AT > Date.now() &&
            token === this.KBASE_AUTH_TOKEN
        ) {
            return this.KBASE_AUTH_INFO;
        }
        this.KBASE_AUTH_TOKEN = token;
        const url = `${this.getServiceOrigin()}/services/auth/api/V2/me`;
        try {
            ...
            this.KBASE_AUTH_INFO = await response.json();
            this.KBASE_AUTH_INFO_EXPIRES_AT = Date.now() + this.KBASE_AUTH_TOKEN_TTL;
            return this.KBASE_AUTH_INFO;
        } catch (error) {
    ...
    async getAuth() {
        ...
        if (token) {
            try {
                const auth = this.fetchAuth(token);
                if (auth) {
                    return auth;
                }
            } catch (ex) {
    ...
    async sendGTag() {
        ...
        const pageView: PageView = {
            ...
            page_path: path,
            page_title: this.LAST_PAGE_TITLE,
        };
        ...
        this.pushGTag('event', 'page_view', pageView);
    ...
    async sendPendingGTag() {
        if (this.isState('PENDING')) {
            ...
            return await this.waitUntilStatus('WAITING', 2000);
        }
    }
    ...
    async sendGTagAfterTitleSettles() {
        this.setState('PENDING');

        let lastChanged = Date.now();
        this.LAST_PAGE_TITLE = document.title;

        // This is where we "wait" for the page title to stabilize,
        // i.e. stop changing.
        const loop = async () => {
            if (this.isState('SEND_NOW')) {
                return;
            }
            const now = Date.now();
            if (now >= lastChanged + this.TITLE_STABILIZATION_PERIOD) {
                return;
            }
            if (document.title !== this.LAST_PAGE_TITLE) {
    ...
    initialize() {
        ...
        this.pushGTag('config', this.GOOGLE_GTAG, {
            send_page_view: false,
        });
        this.pushGTag('config', this.GOOGLE_AD_TAG, {
            send_page_view: false,
        });
        ...
        */
        window.onpopstate = async () => {
            await this.sendPendingGTag();
            this.sendGTagAfterTitleSettles();
        };
...
window.addEventListener('load', () => {
    const gtagSupport = new GTagSupport();
    gtagSupport.initialize();
    // initialize();
