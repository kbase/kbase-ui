...
export default class NarrativeMethodStoreClient extends ServiceClient {
  ...
  async get_method_brief_info(params: GetMethodBriefInfoParams) {
    const [object] = await this.callFunc<[JSONValue], [JSONValue]>('get_method_brief_info', [toJSON(params)]);
    return object as GetMethodBriefInfoResult;
  }

  async list_categories(params: ListCategoriesParams) {
    const object = await this.callFunc<[JSONValue], [JSONValue]>('list_categories', [toJSON(params)]);
    return object as unknown as ListCategoriesResult;
  }

  async get_method_spec(params: GetMethodSpecParams) {
    const [result] = await this.callFunc<[JSONValue], [JSONValue]>('get_method_spec', [toJSON(params)]);
    return result as unknown as GetMethodSpecResult;
  }

  async list_methods(param: ListMethodsParams): Promise<ListMethodsResult> {
    const [result] = await this.callFunc<Array<JSONObject>, Array<JSONArrayOf<JSONObject>>>('list_methods', [
      toJSONObject(param),
    ]);
