...
export type CellType = 'markdown' | 'code_cell' | 'data' | 'kbase_app' | 'widget' | 'serviceWidget';
...
export default class NarrativeHeader extends React.Component<NarrativeHeaderProps, NarrativeHeaderState> {
  ...
  componentDidUpdate(prevProps: NarrativeHeaderProps) {
    if (prevProps.narrativeDoc.access_group !== this.props.narrativeDoc.access_group) {
      this.fetchData();
    }
  }
  ...
  async fetchData() {
    try {
      ...
      const sharedWith = this.props.narrativeDoc.shared_users.filter(
        ...
        (user: string) => {
          return user !== this.props.authInfo.tokenInfo.user && user === '*';
        },
      );
  ...
  countDataTypes(narrative: NarrativeSearchDoc) {
    const typeCounts = narrative.data_objects.reduce<Map<string, number>>((dataTypeCounts, objectInfo) => {
      const typeName = objectInfo.obj_type.split(/[.-]/)[1];
      if (dataTypeCounts.has(typeName)) {
        dataTypeCounts.set(typeName, dataTypeCounts.get(typeName)! + 1);
      } else {
      ...
      <div className="row">
          ...
          <PropTable
            ...
            rows={[
              [
                ...
                () => {
                  return (
                    <UILink path={`people/${narrativeDoc.creator}`} type="kbaseui">
                      {authorName}
                    </UILink>
                  );
                },
              ],
              ['Created on', readableDate(narrativeDoc.creation_date)],
              ['Last saved', readableDate(narrativeDoc.timestamp)],
              ['Version', String(narrativeDoc.version)],
              [
                'Visibility',
                narrativeDoc.is_public ? (
                  <>
                    <Globe style={{ marginRight: '0.25rem' }} />
                    Public
                  </>
                ) : (
                  <>
        ...
        <div className="col-4">
          <PropTable
            // title="Data Objects"
            noRowsMessage="No data objects in this narrative"
            rows={this.countDataTypes(narrativeDoc).map(([key, value]) => {
              return [key, value.toString()];
            })}
            header={['Data Objects', narrativeDoc.data_objects.length.toString()]}
            styles={{
              col2: {
                flex: '0 0 2em',
                textAlign: 'right',
        ...
        <div className="col-3">
          <div className="-prop-table">
            <PropTable
              // title="Cells"
              rows={[
                [cellTypesInfo.kbase_app.label, cellTypesInfo.kbase_app.count.toString()],
                [cellTypesInfo.code_cell.label, cellTypesInfo.code_cell.count.toString()],
                [cellTypesInfo.markdown.label, cellTypesInfo.markdown.count.toString()],
                [cellTypesInfo.data.label, cellTypesInfo.data.count.toString()],
                [cellTypesInfo.widget.label, cellTypesInfo.widget.count.toString()],
                [cellTypesInfo.serviceWidget.label, cellTypesInfo.serviceWidget.count.toString()],
              ]}
              header={['Cells', narrativeDoc.total_cells.toString()]}
              styles={{
                col2: {
                  flex: '0 0 2em',
                  textAlign: 'right',
  ...
  render() {
    return <div className="NarrativeHeader container-fluid">{this.renderState()}</div>;
  }
}
