...
import ServiceWidgetCellView from './cells/ServiceWidgetCell';
...
export default class Preview extends Component<Props, State> {
  ...
  renderFullNarrativeLink(wsid: number) {
    return (
      <UILink className="btn btn-outline-secondary" path={`narrative/${wsid}`} type="europaui" newWindow={true}>
      ...
      case 'code':
        if ('kbase' in cell.metadata) {
          switch (cell.metadata.kbase.type) {
            case 'app':
              return <AppCellView cell={cell as AppCell} authInfo={this.props.authInfo} config={this.props.config} />;
            case 'code':
            ...
            case 'output':
              return <OutputObjectCellVew cell={cell as OutputObjectCell} />;
            case 'serviceWidget':
              return <ServiceWidgetCellView cell={cell as ServiceWidgetCell} />;
          }
        } else {
          ...
          return <UnrecognizedCellView title="Unrecognized" content="Code Cell" />;
        }
    }
  }

  renderNarrative(narrative: NarrativeObject) {
    if (typeof narrative.cells === 'undefined' || narrative.cells.length === 0) {
      return (
        <Row>
          <Col>
            <Empty icon="square-o" message="This Narrative has no cells" />
          </Col>
        </Row>
      );
    }
    ...
    return (
      <>
        {rows}
        <Row className="g-0">{this.renderFullNarrativeLink(this.props.narrative.access_group)}</Row>
      </>
    );
  }
  ...
  render() {
    switch (this.state.loadingState.status) {
      ...
      case AsyncProcessStatus.PENDING:
        return <Loading size="normal" type="block" message="Loading Narrative..." />;
