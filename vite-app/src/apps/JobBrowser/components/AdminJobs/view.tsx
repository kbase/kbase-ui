...
import { Job, JobsSearchExpression, SortSpec, TimeRange, TimeRangePresets } from 'apps/JobBrowser/store';
...
import FilterEditor from '../FilterEditor/controller';
...
/**
 * State for the MuserJobs component
 */
...
export default class AdminJobs extends React.Component<AdminJobsProps, AdminJobsState> {
        ...
        <div style={filterRowStyle}>
          <div className="AdminJobs-filterPanel">
            <div className="AdminJobs-filterArea">
              <FilterEditor filter={this.state.filter} onChange={this.onFilterChange.bind(this)} />
            </div>
          </div>
        </div>
      );
  ...
  renderNarrativeLink(_: JobContextNarrative) {
    return;
  }
  ...
  renderJobsTable() {
    const columns: Array<Column<Job>> = [
...

      {
        ...
        render: (job: Job) => {
          return (
            <UILink path={`people/${job.request.owner.username}`} type="kbaseui" newWindow={true}>
    ...
    return (
      <Table2<Job>
        ...
        loadingPhrase="Loading User Jobs..."
