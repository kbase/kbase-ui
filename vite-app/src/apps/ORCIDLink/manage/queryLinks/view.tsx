...
export default class ORCIDLinkManageView extends Component<ORCIDLinkManageProps, ORCIDLinkManageState> {
  viewLink(username: string) {
    const url = navigationPathToURL({ path: `orcidlink/manage/link/${username}`, type: 'kbaseui' }, true);
  ...
  renderORCIDLink(orcidId: string) {
    return (
      <a href={`${this.props.orcidServiceURL}/${orcidId}`} target="_blank">
        {renderORCIDIcon()}
    ...
    const columns: Array<ColumnDef<LinkRecordPublic>> = [
      {
        ...
        render: (linkRecord: LinkRecordPublic) => {
          return (
            <UILink path={`people/${linkRecord.username}`} type="kbaseui" newWindow={true}>
              {linkRecord.username}
            </UILink>
          );
        },
      ...
      },
      {
        ...
        render: (linkRecord: LinkRecordPublic) => {
          return (
            <Tooltip
              title={Intl.DateTimeFormat('en-US', { dateStyle: 'long', timeStyle: 'long' }).format(
...

      {
        ...
        render: (linkRecord: LinkRecordPublic) => {
          return (
            <Tooltip
              title={Intl.DateTimeFormat('en-US', { dateStyle: 'long', timeStyle: 'long' }).format(
                linkRecord.retires_at,
              )}
            >
              <RelativeTimeClock now={Date.now()} at={linkRecord.retires_at} size="compact" />
            </Tooltip>
          );
        },
      },
        ...
        render: (linkRecord: LinkRecordPublic) => {
          return (
            <Button
              ...
              onClick={() => {
                this.viewLink(linkRecord.username);
              }}
            >
              View
            </Button>
        ...
        render: (linkRecord: LinkRecordPublic) => {
          return (
            <Button
              ...
              onClick={() => {
                this.confirmDeleteLink(linkRecord.username);
              }}
            >
              Delete
            </Button>
  ...
  render() {
    return (
      <div style={{ flex: '1 1 0', display: 'flex', flexDirection: 'column', marginTop: '1rem' }}>
        {this.renderState()}
      </div>
    );
  }
