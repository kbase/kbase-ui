...
import UILinkAntdButton from 'components/UILinkAntdButton';
...
// TODO: move all ant design or bootstrap support types into bespoke files.
...
function Profile(props: ProfileProps) {
  ...
  function renderJobTitleOtherField() {
    return (
      ...
      >
        <Input allowClear={true} placeholder="Job Title" minLength={2} maxLength={50} />
      </Form.Item>
    );
  }
  ...
  function renderDepartmentField() {
    return (
      ...
      >
        <Input allowClear={true} autoComplete="off" minLength={2} maxLength={50} />
      </Form.Item>
    );
  }
  ...
  function renderCityField() {
    return (
      <Form.Item
        ...
        help=""
      >
        <Input allowClear={true} minLength={0} maxLength={85} />
      </Form.Item>
    );
  }
  ...
  function renderPostalCode() {
    ...
    return (
      ...
      >
        <Input allowClear={true} autoComplete="off" minLength={min} maxLength={max} />
      </Form.Item>
    );
  }
  ...
  function renderPrimaryFundingSource() {
    return (
      <Form.Item name="fundingSource" label="Primary Funding Source">
        <Select
          allowClear
          showSearch
          suffixIcon={null}
  ...
  function renderJobTitleField() {
    return (
      <Form.Item label="Position" name="jobTitle">
        <Select placeholder="Job title" allowClear options={jobTitles} />
      </Form.Item>
    );
  }
  ...
  function renderLocation() {
    const location = (
      <div>
        ...
        <Row gutter={8}>
          <Col span={14}>{renderCityField()}</Col>
          <Col span={10}>{renderPostalCode()}</Col>
        </Row>
      </div>
    );
  ...
  function renderSection(title: string, body: string | undefined | JSX.Element) {
    return (
      <div className="Profile-section">
        ...
        <div className="Profile-section-body">{body}</div>
      </div>
    );
  }
              ...
              <div>
                {fields.length === 0
                  ...
                  : fields.map((field, index) => {
                      return (
                        <div className="AffiliationsRow" key={index}>
                          <div className="AffiliationsCol Profile-field-force-inline">
                            ...
                            >
                              <Input type="text" maxLength={50} autoComplete="off" placeholder="Job title" />
                            </Form.Item>
                          </div>
                          <div className="AffiliationsCol Profile-field-force-inline">
  ...
  function renderAvatarOptionsField() {
    return (
      <Form.Item name="avatarOption" label="Avatar Options">
        <Radio.Group>
  ...
  function renderGravatarDefaultField() {
    ...
    const gravatarHelpLink = (
      <a href="https://en.gravatar.com/site/implement/images/#default-image" target="_blank" rel="noopener noreferrer">
        generated or generic image
      </a>
    );
    return (
      <>
        <Form.Item name="gravatarDefault" label="Gravatar Default" style={{ marginBottom: '0' }}>
          <Select options={gravatarDefaults} />
        </Form.Item>
        <div style={{ fontStyle: 'italic' }}>
          If your email address is not registered at{' '}
          <a href="https://gravatar.com" target="_blank">
            gravatar
          </a>
          , the {gravatarHelpLink} you select above will be used instead.
        </div>
      </>
    );
  }
  ...
  function renderAvatarEditor() {
    return (
      <>
        ...
        <div style={{ marginTop: '1rem' }}>{renderAvatarOptionsField()}</div>
        <div style={{ marginTop: '0' }}>{renderGravatarDefaultField()}</div>
      </>
    );
  }
  ...
  function renderAvatar() {
    return (
      <div className="ProfileAvatar">
        <div className="ProfileAvatar-Avatar">{renderAvatarEditor()}</div>
      </div>
    );
  }
  ...
  function renderControls() {
    ...
    const orcidLinkButton2 = (() => {
      if (props.orcidState.status === AsyncProcessStatus.SUCCESS) {
        ...
        return (
          <Tooltip title="Click this button to link your KBase account to your ORCID® account">
            <Button disabled={isFormTouched} onClick={onORCIDLink}>
              Create KBase ORCID® Link...
            </Button>
          </Tooltip>
        );
      }
    ...
    const button = (
      <Space wrap>
        <Button icon={<SaveOutlined />} disabled={!(isFormTouched && isFormValid)} type="primary" onClick={saveForm}>
          {isFormTouched ? 'Save Changes' : 'No Changes to Save'}
        </Button>
        <Button icon={<CloseOutlined />} danger onClick={cancelForm}>
          {isFormTouched ? 'Cancel Changes and Close Editor' : 'Close Editor'}
        </Button>
        <Button danger disabled={!isFormTouched} onClick={resetForm}>
          Reset
        </Button>
        {orcidLinkButton2}
      </Space>
  ...
  function renderResearchInterestsEditor() {
    return (
      <div>
        <Form.Item name="researchInterests" style={{ marginBottom: '0' }}>
          <Checkbox.Group style={{ width: '100%' }}>
            <Row>
              {researchInterestsList.map(({ value, label }) => {
                return (
                  <Col span={24} key={value}>
                    <Checkbox key={value} style={{ marginLeft: '0px' }} value={value}>
                      {label}
                    </Checkbox>
                  </Col>
                );
        ...
        <Form.Item name="researchInterestsOther">
          <Input placeholder="Other research interests" className="margin-top-10px" maxLength={50} />
        </Form.Item>
      </div>
    );
  }
  ...
  function renderORCIDIdLinkEdit(orcidId: string, orcidURL: string) {
    ...
    return (
      <div>
        <div>{renderORCIDIdLink(orcidId, orcidURL)}</div>
        ...
        <div>
          <UILink path="orcidlink" type="kbaseui" newWindow={true}>
  ...
  function renderORCIDIdLink(orcidId: string, orcidURL: string) {
    return (
      <Link
        href={`${orcidURL}/${orcidId}`}
        target="_blank"
        rel="noreferrer"
  ...
  function renderORCIDId(orcidId: string | null, orcidURL: string) {
    if (orcidId) {
      return renderORCIDIdLinkEdit(orcidId, orcidURL);
    }

    const alertMessage = (
      <div>
        Have an ORCID® account? Create an <b>KBase ORCID® Link</b> to connect your KBase to ORCID® account, and show
        your ORCID® iD right here in your profile.
      </div>
    );
    ...
    switch (orcidState.status) {
      ...
      case AsyncProcessStatus.ERROR:
        ...
        return (
          <tr>
            <td colSpan={2}>
              <Alert showIcon type="error" message={orcidState.error.message} />
            </td>
          </tr>
        );
      case AsyncProcessStatus.SUCCESS:
        return (
          <tr>
            <th>ORCID® iD</th>
            <td>{renderORCIDId(orcidState.value.orcidId, orcidState.value.serviceInfo.runtime_info.orcid_site_url)}</td>
          </tr>
        );
    }
  }
  ...
  function renderOrganizations() {
    const message = (
      <div>
        ...
        <p>
          Please visit the
          <UILink path="organizations" type="kbaseui" newWindow={true}>
            Organizations page
          </UILink>{' '}
          to manage your Organization memberships.
        </p>
      </div>
    );
        ...
        <table className="PropertyTable" style={{ alignSelf: 'center' }}>
          <tbody>
            <tr>
              <th>Username</th>
              <td>
                <span data-k-b-testhook-element="username" className="ProfileAvatar-Username">
                  {props.profileView.user.username}
                </span>
              </td>
            </tr>
            {renderORCIDRow()}
  ...
  const affiliations =
    typeof rawAffiliations === 'undefined'
      ? []
      : rawAffiliations
          .filter(({ started, title, organization }) => {
            return title && title.length && organization && organization.length && started;
          })
          .map(({ started, ended, title, organization }) => {
          ...
          >
            <Row gutter={8}>
              <Col span={hasORCIDId() ? 8 : 6}>
                <Well variant="secondary" style={{ flex: '1 1 0' }}>
                  <Well.Header className="justify-content-center">
                    <span className="me-1">{props.profileView.user.realname} </span>
                    <Tooltip title="Use this button to open the Account Manager to edit your display name">
                      <UILinkAntdButton
                        path={{ path: `account`, type: 'kbaseui' }}
                        newWindow={false}
                        type="default"
                        icon={<EditOutlined />}
                      />
                  ...
                  <Well.Body>
                    <div style={{ marginBottom: '1rem' }}>{renderIdentity()}</div>
                    {renderAvatar()}
                  </Well.Body>
                </Well>
              ...
              <Col span={6} style={{ display: 'flex', flexDirection: 'column' }}>
                <Well variant="secondary" style={{ flex: '1 1 0' }}>
                  <Well.Header className="justify-content-center">Research Interests</Well.Header>
                  <Well.Body>{renderResearchInterests()}</Well.Body>
                </Well>
              ...
              <Col span={hasORCIDId() ? 10 : 12} style={{ display: 'flex', flexDirection: 'column' }}>
                <Well variant="secondary" style={{ flex: '1 1 0' }}>
                  <Well.Header className="justify-content-center">KBase Organizations</Well.Header>
                  <Well.Body style={{ flex: '1 1 0', overflowY: 'auto' }}>{renderOrganizations()}</Well.Body>
                </Well>
            ...
            <Row gutter={8} style={{ marginTop: '1rem' }}>
              <Col span={7} style={{ display: 'flex', flexDirection: 'column' }}>
                <Well variant="secondary">
                  <Well.Body>{renderUserNutshell()}</Well.Body>
                </Well>
                {/* <Area >
                                    {renderUserNutshell()}
                                </Area> */}
              ...
              <Col span={7} style={{ display: 'flex', flexDirection: 'column' }}>
                <Well variant="secondary" style={{ flex: '1 1 0' }}>
                  <Well.Header className="justify-content-center">Research or Personal Statement</Well.Header>
                  <Well.Body style={{ flex: '1 1 0', overflowY: 'auto' }}>{renderResearchStatement()}</Well.Body>
                </Well>
              ...
              <Col span={10} style={{ display: 'flex', flexDirection: 'column' }}>
                <Well variant="secondary">
                  <Well.Header className="justify-content-center">Affiliations</Well.Header>
                  <Well.Body>{renderAffiliations()}</Well.Body>
                </Well>
