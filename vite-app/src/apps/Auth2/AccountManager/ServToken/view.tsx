...
export default class ServTokensView extends Component<ServTokensViewProps, ServTokensViewState> {
  ...
  renderTokenBrowser(tokens: Array<TokenInfoFull>, removeVerb: string) {
    const revokeAllButton = (() => {
      if (tokens.length > 1) {
        return (
          <button type="button" className="btn btn-danger" onClick={this.doRevokeAllTokens.bind(this)}>
            {removeVerb} All
          </button>
        );
      }
    ...
    const rows = tokens.map(({ name, id, created, expires, os, osver, agent, agentver, ip }) => {
      const renderBrowser = () => {
        if (agent === null) {
          // } || agent.agent === 0) {  // TODO: what is this actual type???
          return (
            <span
              style={{
                fontStyle: 'italic',
      ...
      const renderOS = () => {
        if (os === null) {
          // } || os.agent === 0) { // TODO: is that supposed to be "length"?
          return (
            <span
              style={{
                fontStyle: 'italic',
      ...
      return (
        <tr key={id}>
          <td>{name}</td>
          <td>{niceTime(new Date(created))}</td>
          <td>{niceDuration(expires - (Date.now() - this.props.serverTimeBias))}</td>
          <td>{renderBrowser()}</td>
          <td>{renderOS()}</td>
          <td>{ip}</td>
          <td>
            <Button
              ...
              type="button"
              onClick={() => {
                this.doRevokeToken(id, name || 'un-named');
              }}
            >
              {removeVerb}
            </Button>
    ...
    return (
      <table className="table table-striped -allTokensTable" style={{ width: '100%' }}>
        <thead>
          <tr>
            <th>Name</th>
            <th>Created</th>
  ...
  renderTokens() {
    return (
      <Well variant="secondary">
        <Well.Header>Service Tokens</Well.Header>
        <Well.Body>{this.renderServTokens()}</Well.Body>
        <Well.Footer>{this.renderRemoveAll()}</Well.Footer>
      </Well>
    );
  }
  ...
  renderHelp() {
    return (
      <div>
        <p>
          a <em>sign-in session</em> is created when you sign in to KBase. A sign-in session is removed when you logout.
          However, if you do not logout, your sign-in session will remain active for two weeks. At the end of two weeks,
          the sign-in session will become invalid, and you will need to sign-in again.
        </p>
      </div>
    );
  }
  ...
  renderNewTokenForm() {
    return (
      <Well variant="secondary" className="mb-4">
        <Well.Header>Create Service Token</Well.Header>
        <Well.Body>
