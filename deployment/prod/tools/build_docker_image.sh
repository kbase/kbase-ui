...
function build_image() {
    ...
    # TODO: should we not label the docker image by the tag not the commit hash?
    docker build --build-arg BUILD_DATE=$date --build-arg VCS_REF=$commit --build-arg BRANCH=$branch --build-arg TAG=$tag -t kbase/kbase-ui:$commit ${here}/docker/context
    err=$?
    if (( $err > 0 )); then
        echo "Error running docker build: $err"
    else
        echo "Successfully build docker image. You may invoke it "
        echo "with tag \"kbase/kbase-ui:${commit}\""
    fi
}
