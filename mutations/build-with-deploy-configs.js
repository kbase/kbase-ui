...
function makeDeployConfig(state) {
    ...
    return fs.mkdirsAsync(cfgDir.join('/'))
        .then(function () {
            ...
            return glob(sourceDir.concat(['*.env']).join('/'), {
                nodir: true
            });
        })
        .then(function (matches) {
            ...
            return Promise.all(matches.map(function (match) {
                ...
                var templatePath = root.concat(['deployment',
                    'ci', 
                    'docker',
                    'kb-deployment',
                    'conf', 
                    'config.json.tmpl']).join('/');

                var outputPath = cfgDir.concat([baseName + '.json']).join('/');
...
function main(type) {
    return Promise.try(function () {
        ...
        var initialFilesystem = [{
            ...
            cwd: ['..'],
            path: ['deployment', 'ci', 'docker', 'kb-deployment', 'conf', 'config.json.tmpl']
        }];
