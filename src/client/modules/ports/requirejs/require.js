...
(function (global, setTimeout) {
    // Amount of time (milliseconds) to pause before running the next "tick"
    // of the event loop.
    const TICK_INTERVAL = 4;

    // The default time in seconds after which a module load is to be considered timed
    ...
    // Can be overridden in the config by the config key "waitSeconds".
    const DEFAULT_WAIT_SECONDS = 7;

    // Interval (ms) with which to check that modules have been loaded, after a load
    // request has been issued. The checkLoaded function will loop until the timeout
    // has expired, or all modules are loaded.
    // This value is used for a setTimeout which gives some breathing room before
    // the next request.
    const CHECK_LOADED_INTERVAL = 50;
    ...
    var req, s, head, baseElement, dataMain, src,
        ...
        isWebWorker = !isBrowser && typeof global.importScripts !== 'undefined',
        ...
        isOpera = typeof global.opera !== 'undefined' && global.opera.toString() === '[object Opera]',
        contexts = {},
        cfg = {},
        globalDefQueue = [],
     ...
     * @param {Array.<string>} requireModules an array of module names
     *
     * @returns {Error}
     */
    function makeError(id, message, err, requireModules) {
    ...
    // Same business here.
    // TODO: There should not be a "requirejs" and a "require" in the global namespace.
    ...
    function newContext(contextName) {
        ...
        // fetched from the cache!! Well, from the registry.
        ...
        function onError(err, errback) {
            ...
            if (errback) {
                ...
                if (!notified) {
                    if (req.onError === defaultOnError && 'onError' in cfg) {
                        cfg.onError(err);
                    } else {
                        req.onError(err);
                    }
        ...
        function checkLoaded() {
            var err, usingPathFallback,
                ...
                // elapsed = context.startTime + waitInterval,
                elapsed = Date.now() - context.startTime,
                expired = waitInterval && elapsed > waitInterval,
                noLoads = [],
                reqCalls = [],
                stillLoading = false,
                needCycleCheck = true;
            ...
            if (expired && noLoads.length) {
                ...
                err = makeError('timeout', `Load timeout after ${elapsed}ms for the following modules: ${noLoads.join(', ')}.`, null, noLoads);
        ...
        Module.prototype = {
            ...
            callPlugin: function () {
                ...
                on(pluginMap, 'defined', bind(this, function (plugin) {
                    ...
                    // EAP
                    // The "onLoad" callback just calls init on the module
                    // with the "factory" arg just returning the value, which
                    // for a plugin is just whatever it provides to it's
                    // onLoad callback.
                    // so if we want to subvert that, we need to intercept the
                    ...
                    if (window.require_resources) {
                        ...
                        const url = localRequire.toUrl(map.name);
     ...
     * EAP - that documentation above is not quite accurate. This is not part of an
     * "event loop". It is simply, as the name suggests, a function which causes another
     * function to execute after the given interval.
     * @param  {Function} fn function to execute later.
     */
    req.nextTick = typeof setTimeout !== 'undefined' ? function (fn) {
        setTimeout(fn, TICK_INTERVAL);
    } : function (fn) {
        fn();
    };
     ...
     */
    req.createScriptNode = function (config) {
     ...
     */
    // EAP - here? here? here is where we need to add the vfs lookup behavior?
    // or create a new load function outside and override this?
    req.load = function (context, moduleName, url) {
        ...
        if (isBrowser) {
            ...
            if (window.require_modules) {

                // look up in vfs
                const reLocalPath = /^(\/.*?)(\?.*)*$/;
                const m = reLocalPath.exec(url);
                if (m) {
                    const realPath = m[1];
                    // var query = m[2];
                    const vfsContent = window.require_modules[realPath];
                    if (vfsContent) {
                        eap_direct_exec = {
                            context: context,
                        ...
                        try {
                            ...
                            console.error('ERROR executing anon mod: ', ex);
                            context.onError(makeError('scripterror',
                                'Error in direct eval of ' + moduleName,
                                ex, [moduleName]));
                        }
                        return;
                    }
                ...
                // if found, use it and simulate load
            ...
            node = req.createScriptNode(config);
    ...
    global.define = function (name, deps, callback) {
        ...
        // currentlyAddingScript is set before the src is set, is picked up here in the
        // define call, which lets us pick up the node directly.
        // Otherwise where does the node come from??
        // The magic little step is in getInteractiveScript().
        // The currently loaded and executing script node is set to readyState 'interactive'
        // The getInteractiveScript function loops through all script tags, and for the
        // one set as interactive, returns it. The node already has the
        // data-requirecontext set on it, which is how we hook back into the original
        // call to set the script src!!!
        // So to hook our local module code into this, we will clearly not hit this
        // mechanism, but we still need to hook the define(..) to the context.
        // Hmm, we can use a similar trick, no?
        if (useInteractive) {
        ...
        // EAP - the eap_direct_exec structure is set during the load call. The load
        // call evaluates the
        if (eap_direct_exec) {
            context = eap_direct_exec.context;
            name = eap_direct_exec.moduleName;
            eap_direct_exec = null;
        }
