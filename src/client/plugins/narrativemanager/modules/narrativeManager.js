...
define([
    ...
    function ($, _, Promise, Workspace, NarrativeMethodStore) {
        'use strict';
        ...
        function factory(config) {
            ...
            function copy_to_narrative(ws_id, importData) {
                return Promise.try(function () {
                    ...
                    return Promise.resolve(workspace.get_object_info(objectsToCopy, 0))
                        .then(function (infoList) {
                            return infoList.map(function (item) {
                                return Promise.resolve(workspace.copy_object({
                                    from: {ref: item}, //!! assume same ordering
                                    to: {wsid: ws_id, name: infoList.shift()[1]}
                                }));
                            });
                        })
            ...
            function complete_new_narrative(ws_id, obj_id, importData, resolve, fail) {
                ...
                return Promise.resolve(workspace.alter_workspace_metadata({
                    wsi: {id: ws_id},
            ...
            */
            ...
            function findRecentValidNarrative(workspaces, resolve, reject) {
                ...
                return Promise.resolve(workspace.get_object_info_new({
                    ...
                    .then(function (objList) {
                        ...
                        // check one workspace at a time to keep the load light
                        if (objList[0] === null) {
                            return findRecentValidNarrative(workspaces, resolve, reject);
                        }
                        resolve({last_narrative: {ws_info: workspace, nar_info: objList[0]}});
                    })
                    .catch(function (err) {
            ...
            function detectStartSettings(resolve, fail) {
                ...
                return Promise.resolve(workspace.list_workspace_info({
                    ...
                    .then(function (wsList) { //only check ws owned by user
                        ...
                        if (workspaces.length > 0) {
                            ...
                            return findRecentValidNarrative(workspaces, 0, resolve, fail);
                        }
                        return {last_narrative: null};
                    });
            }
            ...
            function gatherCellData(cells, specMapping, parameters) {
                var cell_data = [],
                    cellCount = 0;
                console.log('gathering cell data');
                console.log(cells);
                if (cells && cells.length > 0) {
                    cells.forEach(function (cell) {
                        ...
                        if (cell.app) {
                            ...
                            //console.error('cannot add cell # ' + cellCount-1 + ', unrecognized cell content');
                            //console.error(cell);
                            throw {
            ...
            function fetchSpecs(cells) {
                return Promise.try(function () {
                    ...
                    return Promise.resolve(narrativeMethodStore.get_app_spec({ids: appSpecIds}))
                        .then(function (appSpecs) {
                            appSpecs.forEach(function (spec) {
                                var id = appSpecIds.shift();
                                specMapping.apps[id] = spec;
                            });
                            return Promise.resolve(narrativeMethodStore.get_method_spec({ids: methodSpecIds}));
                        })
            ...
            function fetchNarrativeObjects(ws_name, cells, parameters) {
                ...
                return fetchSpecs(cells)
                    .then(function (specMapping) {
                        ...
                        metadataExternal = {};
                        console.log('narrative object is');
            ...
            var introText = _.template(introTemplate)({docBaseUrl: runtime.getConfig('docsite.baseUrl')});
            ...
            function createTempNarrative(params) {
                return Promise.try(function () {
                    ...
                    // 1 - create ws
                    return Promise.resolve(workspace.create_workspace({
